...
def ced(edge_index: torch.Tensor,
    ...
    # edge_weight = (edge_weight.max() - edge_weight) / (edge_weight.max() - edge_weight.min())
    ...
    # print(len(torch.arange(len(sel_mask)).to("cuda")[sel_mask]))
    return edge_index[:, sel_mask], edge_index[:, ~sel_mask]
...
def cav(feature: torch.Tensor,
    ...
    w = torch.tensor(node_cs).to(device)
    ...
    # print(len(torch.arange(len(drop_mask)).to(device)[drop_mask]))
    feature = feature.clone()
    feature[drop_mask] = 0.
    return feature, drop_mask
